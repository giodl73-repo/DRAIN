# Pulse 05: WP-005 `drain-gap` gap analysis (scale-filtered)
...
require an explicit marker (REQ-016). Integrates tier-SLA gaps from `drain-tier`. Creates
`crates/drain-gap/`.

Parent: REQ-008/016 · SPEC-006/013 · PKG-005 · CR-004.

Exit: `cargo test -p drain-gap` green (one found gap at a scale, one null-result case, and
