...
Establish DRAIN's VTRACE planning baseline before any implementation. Author the VTRACE
...
DRAIN is the Wastewater 2.0 build, seventh in the infrastructure-network portfolio after ROUTE
(highways), PYLON (grid), GAUGE (rail), BASIN (water), PACKET (internet), and TARMAC (air) —
completing the sanitation infrastructure complement after road/rail/water/internet/air/ports and, like BASIN/PACKET/TARMAC,
explicitly multi-scale. Doing the V-model left side first means the implementation is governed
by accepted work packages. The invariant this wave establishes: every downstream DRAIN claim
...
|---|---|---|---|---|---|---|
| MISSION | `docs/vtrace/MISSION.md` | settled | full panel (real `.roles`) | 1 minor (permit/consent-decree control) — applied | fixed point reached | CONOPS |
...
| SPECIFICATION_BASELINE | `docs/vtrace/SPECIFICATION_BASELINE.md` | settled | full panel (real `.roles`) | 1 minor (capacity read as control-free) — applied | pass_with_risk; fixed point | TRACE |
