...
VTRACE adoption scope: define verification methods and command levels for DRAIN's requirements.
DRAIN is greenfield, so implementation-dependent checks are `pending` (no code/corpus to run
...
| REQ-004 | schema check / inspection | validate corpus frontmatter keys against schema | stable plant/sewer/network id present; labels are not keys | pending | future EVID-004 |
...
| REQ-007 | analysis / inspection | inspect capacity/adequacy claims for named demand + constraint basis | wet-weather flow-vs-dry-weather flow and WetWeather-vs-DryWeather basis named on each claim | pending | future EVID-007 |
...
| REQ-010 | role review | confirm treatment capacity/effluent quality/overflow/resilience/conveyance/coverage/resource recovery/receiving-water/equity-affordability/benefit-cost/regulatory lenses represented | stakeholder lenses present in `.roles/` and applied | pass_with_risk | EVID-010 (`.roles/` panel built) |
...
| REQ-012 | status inspection | `status review --short`; confirm no TRACKER pointer touched | DRAIN changes stay in the child repo | passed | EVID-012 |
...
# Implementation levels (planned — once drain-network/CLI exist)
# cargo fmt --check
# cargo test
# drain corpus regenerate ; drain tier-sla --gate ; drain gap --scale national
```
...
| EVID-012 | command | `status review --short` (standalone child repo) | REQ-012 | passed |
