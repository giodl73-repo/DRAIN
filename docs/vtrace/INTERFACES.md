...
Interface type: target (greenfield). Controls DRAIN's external and cross-layer boundaries so future work packages cannot change them silently. IF-001..004 restate the `SPECIFICATION_BASELINE.md` public contracts; IF-005..006 add the crate API and CLI introduced in `ARCHITECTURE.md`. None are implemented yet.

## Interface Inventory

| ID | Interface | Type | Owner | Consumers | Compatibility Rule | Verification |
|---|---|---|---|---|---|---|
| IF-001 | Corpus entry file (incl. scale/jurisdiction) | file (markdown + frontmatter) | PKG-002 | analysts, PKG-003/004/005, reviewers | Frontmatter keys additive; `id` immutable; `scale` from a fixed enum | future VER-004/016 / schema check |
...
| IF-005 | `drain-network` library API | API (Rust crate) | PKG-001 | PKG-002..006 | Public types/functions semver; breaking change is change-control | future VER-007 / cargo test |
| IF-006 | `drain` CLI | CLI | PKG-006 | maintainers, agents, analysts | Subcommands/flags additive (incl. `--scale`); output schemas versioned | future VER-001 / command review |

## Interface Details

### IF-001: Corpus entry file

Purpose: one wastewater/sewer-network element as a reviewable, scored, labelled, **scale-tagged** record.

Inputs: frontmatter (`id`, `type`, `scale`, `jurisdiction`, `termini`, `tier`, `sla`, source rows) + a dimension-score block (DIM-01..13).
...
Errors: cited quantity with no registry entry → Citation Auditor finding; permit/ECHO/CWNS proxy used as observed → must be labelled proxy.
...
Inputs: tier definitions, SLA terms (treatment capacity, effluent quality, overflow frequency, service coverage), element tier assignments.
...
### IF-005: `drain-network` library API

Purpose: the graph primitive every layer builds on.
Inputs: `Plant` nodes and `Sewer` edges with attributes and identity, including `capacity_mgd` and typed `DemandBasis` (`WetWeather` or `DryWeather`).
Outputs: graph queries — `add_plant`, `add_sewer`, `plant_count`, `sewer_count`, `degree`, `is_connected`, `has_diverse_path`, and `incident_capacity_mgd`.
...
| Sanitary / Civil Engineer | IF-005 exposes connectivity/capacity as typed queries with error handling, not panics. | pass |

Fixed-point note: no actionable finding required a change; interfaces are consistent with SPEC public contracts and architecture boundaries. No unresolved critical/major finding. Schema, output-format, and scale-nesting details deferred to IFQ-001..004.
