...
Implementation baseline: the accepted left-side VTRACE artifacts. DRAIN is greenfield; this plan
...
Build the pipeline primitive-first. The wastewater/sewer graph kernel (`drain-network`) has no
internal dependencies and carries the load-bearing invariants (identity, connectivity), so it
ships first with a Cargo workspace. The corpus layer adds the scale/jurisdiction tags. Each subsequent
...
|---:|---|---|---|---|
| 1 | Wastewater/sewer graph kernel: identity, connectivity, centrality, capacity and demand-basis helpers | `Cargo.toml`, `crates/drain-network/` | WP-001 | Primitive; no deps; carries invariants. |
| 2 | Corpus + scale/jurisdiction tags + source registry + schema + evidence labels | `crates/drain-corpus/`, `corpus/SCHEMA.md`, `data/sources.md` | WP-002 | Depends on kernel types; adds scale; feeds all scoring. |
| 3 | Dimension scoring (DIM-01..13) + rubric version record | `crates/drain-score/` | WP-003 | Depends on corpus; needed before tier/gap. |
| 4 | Tier classification (T1–T4) + SLA conformance (DIM-13) + tier-SLA gap | `crates/drain-tier/` | WP-004 | Depends on score + kernel. |
| 5 | Gap analysis (under-served regions, scale filter, null result) | `crates/drain-gap/` | WP-005 | Depends on score + tier. |
| 6 | `drain` CLI orchestration (incl. `--scale`) + reproducible artifacts | `crates/drain-cli/` | WP-006 | Orchestrates all layers; last. |
...
|---|---|---|---|
| PKG-001 / IF-005 | WP-001 | `crates/drain-network/**`, workspace `Cargo.toml` | no |
| PKG-002 / IF-001 / IF-002 | WP-002 | `crates/drain-corpus/**`, `corpus/`, `data/` | yes (kernel types) |
| PKG-003 / IF-003 | WP-003 | `crates/drain-score/**` | yes (corpus) |
| PKG-004 / IF-004 | WP-004 | `crates/drain-tier/**` | yes (score, kernel) |
| PKG-005 | WP-005 | `crates/drain-gap/**` | yes (score, tier) |
| PKG-006 / IF-006 | WP-006 | `crates/drain-cli/**` | yes (all) |
...
Active wave: a new `context/waves/<date>-drain-implementation/` wave (created when WP-001
...
|---|---|---|---|---|
| WP-001 | Wastewater/sewer graph kernel | `crates/drain-network` | `cargo test -p drain-network` | evidence/trace/review/status rows |
| WP-002 | Corpus + scale + sources + schema | `crates/drain-corpus`, `corpus/`, `data/` | `cargo test -p drain-corpus`, `proof check .` | evidence/trace rows |
| WP-003 | Dimension scoring | `crates/drain-score` | `cargo test -p drain-score` | evidence/trace rows |
| WP-004 | Tier + SLA conformance | `crates/drain-tier` | `cargo test -p drain-tier` | evidence/trace rows |
| WP-005 | Gap analysis (scale-filtered) | `crates/drain-gap` | `cargo test -p drain-gap` | evidence/trace rows |
| WP-006 | CLI orchestration | `crates/drain-cli` | `cargo run -p drain-cli -- --help`, `cargo test` | evidence/trace rows |

Boundary rule: VTRACE/work-package/proof/readiness concepts are **not** product features. DRAIN
...
| L2 | Pipeline readiness | `drain` end-to-end run at a chosen scale on a seed corpus + role review | first public claim |

## Risks

| Risk ID | Risk | Mitigation | Owner |
|---|---|---|---|
| RISK-001 | Scope creep inside a work package. | Each WP has fixed exit criteria + boundary control. | DRAIN maintainer |
| RISK-002 | Premature scoring before calibration. | WP-003 ships provisional scores, labelled; calibration is a later wave. | DRAIN maintainer |
| RISK-003 | Data acquisition / permit/ECHO noise bleeds into WP-002. | WP-002 ships schema + fixtures only; real acquisition (FLETCH) deferred; permit/ECHO labelled proxy. | data steward |
| RISK-004 | Scale nesting (DEF-005) undecided. | WP-002 ships a flat `scale` enum; nesting is a later change-controlled extension. | DRAIN maintainer |
