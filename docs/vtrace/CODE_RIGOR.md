...
Repo: DRAIN (all `drain-*` crates)

Risk level: medium (research tooling with critical graph/identity/label/scale logic; no safety-of-life, but wrong capacity, effluent, overflow, or SLA claims are harmful if overstated).
...
|---|---|---|---|---|
| CR-001 | Functions stay small enough for focused review; soft cap 60 logical lines. | All `drain-*` code | review + `cargo clippy` (`too_many_lines`) | Larger units need a rationale comment. |
...
| CR-006 | No `unwrap`/`expect`/`panic!` in library code paths except tests and documented invariants. | `drain-network/corpus/score/tier/gap` libs | `cargo clippy`, review | Allowed in `drain-cli` top-level error reporting and tests. |
...
| Area | Rule | Rationale |
|---|---|---|
| Errors | Each lib crate defines a `thiserror` error enum; `drain-cli` uses `anyhow`. | Typed library errors (CR-003), ergonomic CLI. |
| Numbers | Capacity = MGD; effluent concentration = mg·L⁻¹; overflow frequency = events·yr⁻¹; cost = $; asset age = days/years; scores are `f64` in `[0,10]`. | Numeracy discipline (editorial gate). |
...
| Demand basis | Capacity/adequacy values carry typed `DemandBasis` (`WetWeather` or `DryWeather`), not prose. | Enforce REQ-007/SPEC-005 at the type level. |
...
| Operations Officer | The demand-basis tailoring rule pins WetWeather/DryWeather into the type system, not prose. | pass |
| Scope Keeper | The scale tailoring rule makes scale a typed enum and cross-scale ops explicit. | pass |
| Citation Auditor | CR-007 makes label/source/basis/scale preservation a hard invariant. | pass |
